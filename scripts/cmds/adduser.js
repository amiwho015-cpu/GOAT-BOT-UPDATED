...
module.exports = {
  ...
  onStart: async function ({ message, api, event, args, threadsData, getLang }) {
    ...
    for (const item of args) {
      ...
      // Try to resolve name from group members list first, then API
      const memberInGroup = members.find(m => m.userID == uid);
      if (memberInGroup?.name) {
        userName = memberInGroup.name;
      } else {
        try {
          const info = await api.getUserInfo(uid);
          userName = info?.[uid]?.name || `UID: ${uid}`;
        } catch {
          userName = `UID: ${uid}`;
        }
      }
